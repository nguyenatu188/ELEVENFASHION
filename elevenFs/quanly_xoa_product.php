...
<body>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (isset($_POST['delete_product'])) {
            $product_name_to_delete = $_POST['product_name_to_delete'];

            // Thực hiện truy vấn để xóa sản phẩm
            $delete_query = "DELETE FROM product WHERE pName = '$product_name_to_delete'";
    ...
    <button type="button" class="btn btn-secondary" onclick="location.href='Quanlysanpham.php';">Trở về</button>
                ...
                <br>
        <div class="mb-3">
            <label for="product_name_to_delete" class="form-label">Nhập tên sản phẩm cần xóa:</label>
            <input type="text" class="form-control" id="product_name_to_delete" name="product_name_to_delete" required>
        </div>

        <button type="submit" name="delete_product" class="btn btn-secondary">Xóa sản phẩm</button>
    </form>
